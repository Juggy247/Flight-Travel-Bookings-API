...
# --- Fixtures ---
@pytest.fixture(autouse=True)  # ✅ added — resets DB before every test
def reset_db():
...
def test_update_flight_price():
    ...
    assert response.json()["name"] == "AirAsia"  # other fields unchanged
...
def test_pagination():
    ...
    for i in range(15):
        create_flight(token, {
            ...
            "boarding_time": f"2026-06-{i+1:02d}T10:00:00"  # ✅ different date each time
        })
